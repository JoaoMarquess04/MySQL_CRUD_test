...
(10, '2020-10-10', 23000.60, 9000, 1056),
  ...
  select * from produtos;
    ...
    SELECT AVG(quotaVendas) AS quotaMedia FROM vendedores;
 
	SELECT COUNT(*) AS quantidadeVendedores FROM vendedores;
    
    SELECT nomeVendedor, quotaVendas
	FROM vendedores
	WHERE quotaVendas = (SELECT MAX(quotaVendas) FROM vendedores);
    
    SELECT nomeVendedor, quotaVendas
	FROM vendedores
	WHERE quotaVendas = (SELECT MIN(quotaVendas) FROM vendedores);
    
    SELECT SUM(quotaVendas) AS somaQuotas
	FROM vendedores;
    ...
    SELECT SUM(valorPedido) AS valorTotalPedidos
FROM pedidos
WHERE idVendedor = 1002;


SELECT MIN(dataPedido) AS pedidoMaisAntigo
FROM pedidos;
...
SELECT idVendedor, AVG(valorPedido) AS mediaValoresPedidos
FROM pedidos
GROUP BY idVendedor;


SELECT idVendedor, COUNT(*) AS totalPedidos
FROM pedidos
GROUP BY idVendedor;


SELECT idVendedor, MAX(valorPedido) AS maxValorPedido, MIN(valorPedido) AS minValorPedido
FROM pedidos
GROUP BY idVendedor;
...
SELECT idVendedor, COUNT(*) AS quantidadePedidos
FROM pedidos
WHERE valorPedido > 1000.00
GROUP BY idVendedor
ORDER BY idVendedor;
...
SELECT
  ...
  MIN(valorPedido) AS valorMinimoPedido,
  MAX(valorPedido) AS valorMaximoPedido
FROM pedidos
WHERE valorPedido > 1000.00
GROUP BY idVendedor
ORDER BY idVendedor;
...
SELECT
  idVendedor,
  AVG(valorPedido) AS valorMedioPedido
FROM pedidos
WHERE idVendedor IN (
  SELECT idVendedor
  FROM pedidos
  GROUP BY idVendedor
  HAVING SUM(valorPedido) > 20000
)
GROUP BY idVendedor;
